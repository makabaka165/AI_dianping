...
class ShopReviewEvidenceRetrieverTest {
    ...
    void retrieveShouldMergeVectorEvidenceWhenReviewRagEnabled() {
        ...
        when(blogMapper.selectQualityBlogsByShopId(eq(10L), eq(0), eq(5))).thenReturn(Collections.singletonList(highLiked));
        when(blogMapper.selectRecentBlogsByShopId(eq(10L), eq(5))).thenReturn(Collections.emptyList());
        when(blogMapper.selectNegativeCandidateBlogsByShopId(eq(10L), eq(3))).thenReturn(Collections.emptyList());
