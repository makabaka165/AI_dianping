...
public class HttpToolProtocolAdapter implements ToolProtocolAdapter {
    ...
    @Override public ToolResult execute(ToolDefinition definition,ToolInvocation invocation,JsonNode config){long started=System.currentTimeMillis();try{
        ...
        OutboundHttpResponse response=http.execute(new OutboundHttpRequest(uri,method,headers,body,Duration.ofMillis(definition.getTimeoutMs()),Math.max(1024,config.path("maxResponseBytes").asInt(1024*1024)),contentTypes(config),config.path("allowPrivateNetwork").asBoolean(false)));
    ...
    }catch(Exception e){String code=code(e,"HTTP_TOOL_FAILED");return ToolResult.failure(ToolCallStatus.FAILED,code,"HTTP tool execution failed",retryable(code));}}
