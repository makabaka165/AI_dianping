...
public class IntentFusionService {
    ...
    public IntentFusionService(IntentRuleClassifier rules, IntentModelClassifier model,
                               IntentConfidencePolicy confidencePolicy, SlotFillingService slots) {
        this.rules = rules;
        this.model = model;
    ...
    public IntentClassification classify(String text, Map<String, Object> entities,
        ...
        if (confidencePolicy.shouldUseModel(rule.getConfidence())) {
            Optional<IntentClassification> modelResult = model.classify(text, entities,
                    context.getExecutionContext(), context.getAgent(), context.getNodeRunId());
