...
public class ToolExecutionPipeline {
    ...
    public ToolResult execute(ToolInvocation invocation) {
        ...
        ToolDefinition definition = definitions.findBound(invocation.getContext().getTenantId(),
                ...
                invocation.getContext().getAgentVersion(), invocation.getToolCode(), invocation.getToolVersion())
                .orElse(null);
        if (definition == null) {
            return ToolResult.failure(ToolCallStatus.DENIED, "TOOL_NOT_BOUND",
    ...
    private ToolResult executeAuthorized(ToolDefinition definition, ToolInvocation invocation, long started) {
        ...
        if (definition.isIdempotent()) {
            Optional<JsonNode> cached = idempotency.find(invocation.getContext().getTenantId(),
                    invocation.getContext().getWorkspaceId(), idempotencyKey);
            if (cached.isPresent()) return ToolResult.success(cached.get(), System.currentTimeMillis() - started);
        }
        ...
        if (budgetTtl.isNegative() || budgetTtl.isZero()) {
            return ToolResult.failure(ToolCallStatus.TIMED_OUT, "RUN_DEADLINE_EXCEEDED",
                    "run deadline exceeded", false);
        }
        if (!budgets.reserve(invocation.getContext().getTenantId(), invocation.getContext().getWorkspaceId(),
                invocation.getContext().getRunId(), invocation.getContext().getExecutionBudget().getMaxToolCalls(),
                budgetTtl)) {
            return ToolResult.failure(ToolCallStatus.DENIED, "TOOL_BUDGET_EXCEEDED",
        ...
        if (!rateLimits.acquire(invocation.getContext().getTenantId(), definition.getId(), permits)) {
            return ToolResult.failure(ToolCallStatus.DENIED, "TOOL_RATE_LIMITED",
        ...
        if (result.getStatus() == ToolCallStatus.SUCCEEDED && definition.isIdempotent()) {
            ...
            idempotency.store(invocation.getContext().getTenantId(), invocation.getContext().getWorkspaceId(),
                    idempotencyKey, result.getData(), Duration.ofSeconds(seconds));
        }
        return result;
    }
